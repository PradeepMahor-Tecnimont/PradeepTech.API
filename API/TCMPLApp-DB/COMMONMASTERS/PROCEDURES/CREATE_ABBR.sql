--------------------------------------------------------
--  DDL for Procedure CREATE_ABBR
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "COMMONMASTERS"."CREATE_ABBR" IS
...
--CURSOR C1 IS SELECT * FROM LIST1 WHERE  empno = '11582' ORDER BY EMPTYPE DESC, DOJ ASC FOR UPDATE;
 ...
 --CURSOR C1 IS SELECT * FROM EMPLMAST WHERE EMPTYPE = 'R' and status =1 and NEW_SHORTCD = ' ' ORDER BY EMPTYPE DESC, DOJ ASC FOR UPDATE;
 CURSOR C1 IS SELECT EMPNO,EMPTYPE,COMPANY,LASTNAME,FIRSTNAME,MIDDLENAME,ABBR FROM EMPLMAST WHERE COMPANY = 'TICB' AND EMPTYPE = 'R' and status =1 and ABBR = ' ' ORDER BY EMPTYPE DESC, DOJ ASC FOR UPDATE;
BEGIN
 ...
 --  DBMS_OUTPUT.put_line('hello '||v_record.empno);
       ...
       END IF;
       IF trim(V_RECORD.middlename)  is not null THEN
          V_SHORTCD := V_SHORTCD || SUBSTR(V_RECORD.MIDDLENAME,1,1);
          V_SHORTCD_M := SUBSTR(V_RECORD.MIDDLENAME,1,1);
       END IF;
      ...
      -- DBMS_OUTPUT.put_line(v_record.empno ||  v_record.firstname || v_record.lastname || V_SHORTCD ||V_SHORTCD_F || V_SHORTCD_M || V_SHORTCD_L);
       ...
       IF SHORTCD_FinD(V_SHORTCD) = 0 THEN
          --UPDATE LIST1 SET NEW_SHORTCD = V_SHORTCD WHERE CURRENT OF C1;
            UPDATE emplmast  SET ABBR = V_SHORTCD WHERE CURRENT OF C1;
