Create Or Replace Package Body commonmasters.pkg_obj_grants_to_orcl_users As
    ...
    tab_select_grants typ_tab_obj_account:= typ_tab_obj_account(
            rec_obj_account('projmast', 'desk_book'),
            rec_obj_account('holidays', 'desk_book')
        );
        ...
        Loop
            ...
            If v_exception Is Null Then
                tcmpl_app_config.task_scheduler.sp_log_success(
                    'COMMONMASTERS.GRANT_' || p_grant_name,
                    v_statement
                );
            Else
