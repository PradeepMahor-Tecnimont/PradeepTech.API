--------------------------------------------------------
--  DDL for Function CLOSINGOHBAL
--------------------------------------------------------

  CREATE OR REPLACE FUNCTION "SELFSERVICE"."CLOSINGOHBAL" (V_EmpNo  In Varchar2, V_OpBalDtFrom IN Date, V_OpenBal IN Number) Return Number IS
	VCount number;
	VCount1 number;
	VCount2 number;
	...
	V_OpBalDtFetched := to_date('1-jan-1900','dd-mon-yyyy');
  	...
  	If V_OpBalDtFrom is null then
	  	If VCount1 > 0 then
	  		-- Get date and 4 leave balances from LeaveBal Table  and return OpBalDtFetched and 4 LeaveBals
				select  BalDate, nvl(OH_Bal,0)
				into V_OpBalDtFetched ,V_OH 
				...
				select count(*) into VCount2 from ss_leaveledg where empno = V_Empno 
					...
					If VCount2 > 0 then
						Select V_OH + Sum(LeavePeriod) into V_OH from SS_LeaveLedg 
							where empno = trim(V_EmpNo) and BDate >= V_OpBalDtFetched and leavetype='OH' 
							group by leavetype;
					End If;
  	Else
	  ...
	  If V_OpenBal = 0 and V_OpBalDtFrom is not null then
				...
				select count(*) into VCount2 from ss_leaveledg where empno = V_Empno 
					...
					If VCount2 > 0 then
						Select V_OH + Sum(LeavePeriod) into V_OH from SS_LeaveLedg 
							where empno = trim(V_EmpNo) and BDate >= V_OpBalDtFetched and BDate <= V_OpBalDtFrom and leavetype='OH' 
							group by leavetype;
					End If;
