--------------------------------------------------------
--  DDL for Function CLOSINGPLBAL
--------------------------------------------------------

  CREATE OR REPLACE FUNCTION "SELFSERVICE"."CLOSINGPLBAL" (V_EmpNo  In Varchar2, V_OpBalDtFrom IN Date, V_OpenBal IN Number) Return Number IS
	VCount number;
	VCount1 number;
	VCount2 number;
				...
				select count(*) into VCount2 from ss_leaveledg where empno = V_Empno 
					...
					If VCount2 > 0 then
						Select V_PL + Sum(LeavePeriod) into V_PL from SS_LeaveLedg 
							where empno = trim(V_EmpNo) and BDate >= V_OpBalDtFetched and leavetype='PL' 
							group by leavetype;
					End If;
  	Else
