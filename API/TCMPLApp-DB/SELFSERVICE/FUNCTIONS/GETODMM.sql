--------------------------------------------------------
--  DDL for Function GETODMM
--------------------------------------------------------

  CREATE OR REPLACE FUNCTION "SELFSERVICE"."GETODMM" (p_AppNo IN Varchar2, p_Time IN Number) RETURN Number IS
		v_Button Number;
...
BEGIN
  	...
  	If v_ODRow.ODType = 1 Then
				Return Nvl(v_ODRow.MM,0);
  	ElsIf v_ODRow.ODType = 2 Then
  			If p_Time = 1 Then
  					Return Nvl(v_ODRow.MM,0);
  			ElsIf p_Time = 2 Then
  					Return Nvl(v_ODRow.MM1,0);
  			End If;
