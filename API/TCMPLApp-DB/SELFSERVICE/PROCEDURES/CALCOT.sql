--------------------------------------------------------
--  DDL for Procedure CALCOT
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "SELFSERVICE"."CALCOT" (EmpNum IN Varchar2, V_PDate IN Date, ITime OUT Number, V_ITime OUT Varchar2,
...
BEGIN
		...
		End If;
		select count(*) into PunchNos from ss_ot where empno = ltrim(rtrim(EmpNum)) and PDate = V_PDate;
		If PunchNos > 1 then
				...
				Else
					...
					--If LCome + EGo > 2 * 60 Then
						--EStHrs := EStHrs / 2;
						--V_Absent := 0.5;
						OTHrs := WorkedHrs1(trim(EmpNum),V_PDate,Trim(SCode)) - EStHrs;
					/*ElsIF LCome + EGo > 4 * 60 Then
						--EStHrs := 0;
						V_Absent := 1;
						OTHrs := WorkedHrs1(trim(EmpNum),V_PDate) - EStHrs;
					Else
						OTHrs := ECome + LGo;
					End If;*/
					If OTHrs > 0 Then
					...
					ElsIf OTHrs < 1 Then
						/*OTHrs := 0;
						V_OTHrs := ' ';*/
						V_OTHrs := lpad(to_char(Floor((OTHrs*-1)/60)),2,'0')|| ':' || lpad(to_char(Mod((OTHrs*-1),60)),2,'0');
					End If;
				End If;
		End if;
END
