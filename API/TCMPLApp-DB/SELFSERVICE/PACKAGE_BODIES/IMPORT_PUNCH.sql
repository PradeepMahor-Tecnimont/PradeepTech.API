...
Create Or Replace Package Body selfservice.import_punch As
    ...
    Procedure manage_repetitive_punch(
        p_date Date
    ) As

        Cursor cur_repeat_punch Is
            ...
            From
                (
                    ...
                    Select
                        ...
                        punch_time,
                        mach,
                        Lag(punch_time, 1, 0) Over ( Partition By empno, pdate Order By empno,
                                pdate,
                                punch_time) prev_punch,
                        Lead(punch_time, 1, 0) Over ( Partition By empno, pdate Order By empno,
                                pdate,
                                punch_time) next_punch,
                        punch_mm,
                        Lag(punch_mm, 1, 0) Over ( Partition By empno, pdate Order By empno,
                                pdate,
                                punch_mm)   prev_punch_mm,
                        Lead(punch_mm, 1, 0) Over ( Partition By empno, pdate Order By empno,
                                pdate,
                                punch_mm)   next_punch_mm,
                        office,
                        Lag(office, 1, 0) Over ( Partition By empno, pdate Order By empno,
                                pdate,
                                punch_mm)   prev_office
                    ...
                    From
                        (
                            ...
                            Select
                                ...
                                pdate,
                                mach,
                                ((hh * 60 * 60) + (mm * 60) + ss) As punch_time,
                                hh * 60 + mm                      As punch_mm,
                                office
                            From
                                ss_punch           p,
                                ss_swipe_mach_mast m
                            Where
                                pdate      = p_date

                                And p.mach = m.mach_name
                                And m.office_id In ('4', '5')
                            Order By empno, pdate, hh, mm, ss
                        )
                ), params

            Where
                (punch_mm - prev_punch_mm) Between min_mm And max_mm
                And office = prev_office;

        Type typ_tab_repeat_punch Is Table Of cur_repeat_punch%rowtype;
        tab_repeat_punch typ_tab_repeat_punch;
        v_pdate          Date;
    Begin

        Open cur_repeat_punch;
        Loop
            Fetch cur_repeat_punch Bulk Collect Into tab_repeat_punch Limit 100;
            For i In 1..tab_repeat_punch.count
            Loop
                Begin
                    Update
                        ss_punch
                    Set
                        falseflag = 0
                    Where
                        empno                                 = tab_repeat_punch(i).empno
                        And pdate                             = p_date
                        And ((hh * 60 * 60) + (mm * 60) + ss) = tab_repeat_punch(i).punch_time;
                Exception
                    When Others Then
                        Null;
                End;
            ...
            Exit When cur_repeat_punch%notfound;
        End Loop;
    Exception
        When Others Then
            Null;
    ...
    Begin
        ...
        param_success := 'OK';
        param_message := 'Punch Upload and Normalization has been scheduled.';
    Exception
        When Others Then
            param_success := 'KO';
            param_message := 'Err - ' || sqlcode || ' - ' || sqlerrm;
    ...
    Begin
        ...
        --
        --manage repetitive punch of airoli emp
        Declare
            Cursor cur_punch_dates Is
                Select
                Distinct dd || mon || yyyy str_date
                From
                    ss_importpunch;
            v_pdate Date;
        Begin
            For c1 In cur_punch_dates
            Loop
                v_pdate := To_Date(c1.str_date, 'ddmmyyyy');
                --disable repetitive punch
                manage_repetitive_punch(v_pdate);
                
                --add missed punch application for newly joined employees
                od.gen_first_day_missed_punch(v_pdate);
            End Loop;
        End;
        --
        --
