--------------------------------------------------------
--  DDL for Procedure INS_NEW_PRJ_FORBUDG_INPROJMAST
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "TIMECURR"."INS_NEW_PRJ_FORBUDG_INPROJMAST" AS
cursor c1 is select distinct projno from budgmast where projno not in (select projno from projmast);
 ...
 open c1;
  loop
      ...
      loop
       ...
       --  dbms_output.put_line('projno ' || test_projno);
      end loop;
      close c2;
      commit;
  end loop;
