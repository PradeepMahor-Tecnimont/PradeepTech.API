--------------------------------------------------------
--  DDL for Procedure GENERATE_DELTA_INVOICES
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "TIMECURR"."GENERATE_DELTA_INVOICES" (p_processing_month in timetran.yymm%type) IS
   ...
   cursor c_inv_delta is select inv_invoice_master.rowid,inv_invoice_master.* from inv_invoice_master,projmast  
   where inv_invoice_master.projno = projmast.projno and nvl(projmast.excl_delta_billing,0) = 0 
   and inv_invoice_master.overbudget > 0 and 
   nvl(inv_invoice_master.invoiced_current,0)-
   (nvl(inv_invoice_master.rollback_overbudget,0)+nvl(inv_invoice_master.rollback_current,0)) > 0 
   and nvl(inv_invoice_master.delta_inv,0) = 0 and nvl(overbudget,0) = 1 
   order by inv_invoice_master.projno,inv_invoice_master.costcode,
   inv_invoice_master.suborder_no,inv_invoice_master.loa_no;
   
   --- Get balance loa's
   cursor c_loa_update(p_projno char,p_costcode char) is select * from inv_loa_details 
   where projno = p_projno and costcode = p_costcode and loa_closed <> 1 
   and nvl(budgetted_hours,0)-(nvl(ticbconsumed_hours,0)+nvl(subcconsumed_hours,0) + 
   nvl(delta_hours,0)+nvl(delta_current,0)) > 0 for update;

begin
	 -- delete from inv_workfile1;
	 update inv_invoice_master set rollback_current = 0;
	 update inv_loa_details set delta_current = 0,delta_current_amount = 0,
	 rollback_current = 0,rollback_current_amount = 0;
	 commit;
	 v_rates := 0;
	 For invoices In c_inv_delta Loop
           ...
           if v_invyymm < '200104' then
              ...
              where projno = loas.projno and costcode = loas.costcode;
           exception
           ...
           select tcmno into v_tcm_no from projmast where projno = invoices.projno;
    	   	 ...
    	   	 if v_invoice_now <> 0 then
	 	   	      ...
	 	   	      update inv_invoice_master set rollback_current = nvl(rollback_current,0) + v_invoice_now where rowid = invoices.rowid;
