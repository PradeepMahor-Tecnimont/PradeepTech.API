--------------------------------------------------------
--  DDL for Procedure INS_NEW_PRJ_FOROPEN_INPROJMAST
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "TIMECURR"."INS_NEW_PRJ_FOROPEN_INPROJMAST" AS
cursor c1 is select distinct projno from OPENMASTCOPY where projno not in (select projno from projmast);
cursor c2 (vProjno char) is select * from projmast where rownum = 1 and substr(projno,1,5) = substr(vProjno,1,5) and (cdate is null OR (CDATE >= '01-JAN-2011' AND CDATE <= '31-MAR-2011' ) ) order by projno;
c1_rec c1%rowtype;
c2_rec c2%rowtype;
test_projno CHAR(7);
vCnt  number(6);
BEGIN
  ...
  loop
      ...
      --   vCnt := vCnt + 1 ;
    --    dbms_output.put_line('projno ' || test_projno || ' count' || (vCnt) ); 
      end loop;
      close c2;
      commit;
  end loop;
