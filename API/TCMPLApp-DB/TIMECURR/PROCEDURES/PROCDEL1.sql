--------------------------------------------------------
--  DDL for Procedure PROCDEL1
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "TIMECURR"."PROCDEL1" (p_projno Varchar2, p_sid Varchar2, p_month Varchar2)   
IS   
   	vProjNo   char(5); 
   	vPhase    char(2);	 
   	vSid      char(80); 
...
BEGIN   
    ...
    Insert into job_budgtran (projno,costcode,yymm,initbudg,revbudg,phase)  
      Select projno,costcode,yymm,initbudg,revbudg,phase From tp_xls  
        Where Projno = vProjNo and Sessionid = vSid and (initbudg <> 0 or revbudg <> 0) ;
    Commit;	
        ...
        Begin
          ...
          (Select a.*, rownum rnum From tp_xls a Where Projno = vProjNo and Sessionid = vSid  
            and yymm >= vMonth and costcode = c1.costcode Order by rnum desc) 
              Where  rnum >= 13 Group by costcode;
          If v_RevBal > 0 Then
            v_RevBal := v_RevBal;
          End If;
        Exception
