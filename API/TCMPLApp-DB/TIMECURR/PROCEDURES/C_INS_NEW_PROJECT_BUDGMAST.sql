--------------------------------------------------------
--  DDL for Procedure C_INS_NEW_PROJECT_BUDGMAST
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "TIMECURR"."C_INS_NEW_PROJECT_BUDGMAST" AS
cursor c1 is (select distinct projno,newco from budgmast_COPY where PROJNO NOT LIKE '03287%' AND projno||newCO not in (select projno||CO from projmast) );
