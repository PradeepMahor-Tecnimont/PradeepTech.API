--------------------------------------------------------
--  DDL for Procedure OLD_GENERATE_WORKFILE_FINAL
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "TIMECURR"."OLD_GENERATE_WORKFILE_FINAL" (p_processing_month in timetran.yymm%type) IS
   ...
   cursor c_cumm(p_yymm char) is select a.projno,a.costcode,sum(nvl(a.ticb_hrs,0.00)) as ticbhrs,sum(nvl(a.subc_hrs,0.00)) as subchrs from inv_timetran_tcm a,projmast b where a.projno = b.projno and nvl(b.excl_billing,0) = 0 and a.yymm <= p_yymm group by a.projno,a.costcode order by 1,2;
...
*/         
         ...
         while v_projno = v_loopprojno loop
              ...
              v_rates4 := 0.00;
              begin
                  select a.rate_code1,nvl(b.rates,0.00) as rates1,a.rate_code2,nvl(c.rates,0.00) as rates2,a.rate_code3,nvl(d.rates,0.00) as rates3,a.rate_code4,e.rates as rates3 into
                  v_rate_code1,v_rates1,v_rate_code2,v_rates2,v_rate_code3,v_rates3,v_rate_code4,v_rates4 
                  from inv_proj_dept_ratemaster a,inv_ratemaster b,inv_ratemaster c,inv_ratemaster d,inv_ratemaster e where a.projno = v_loopprojno and a.costcode = v_loopcostcode 
                  and a.rate_code1 = b.rate_code and a.rate_code2 = c.rate_code and a.rate_code3 = d.rate_code and a.rate_code4 = e.rate_code; 
              exception
                  when no_data_found then
                       begin
                          select a.rate_code1,nvl(b.rates,0.00) as rates1,a.rate_code2,nvl(c.rates,0.00) as rates2,a.ratecode3,nvl(d.rates,0.00) as rates3,a.ratecode4,nvl(e.rates,0.00) as rates3 into
                          v_rate_code1,v_rates1,v_rate_code2,v_rates2,v_rate_code3,v_rates3,v_rate_code4,v_rates4 
                          from inv_dept_ratemaster a,inv_ratemaster b,inv_ratemaster c,inv_ratemaster d,inv_ratemaster e where a.costcode = v_loopcostcode 
                          and a.rate_code1 = b.rate_code and a.rate_code2 = c.rate_code and a.ratecode3 = d.rate_code and a.ratecode4 = e.rate_code; 
                       exception
                          when no_data_found then
                               v_rate_code1 := 'Z1';
                               select rates into v_rates1 from inv_ratemaster where rate_code = v_rate_code1;
                               v_rate_code2 := 'Z2';
                               select rates into v_rates2 from inv_ratemaster where rate_code = v_rate_code2;
                               v_rate_code3 := 'Z3';
                               select rates into v_rates3 from inv_ratemaster where rate_code = v_rate_code3;
                               v_rate_code4 := 'Z4';
                               select rates into v_rates4 from inv_ratemaster where rate_code = v_rate_code4;
                       end;
              end;
              ...
              while v_projno = v_loopprojno and v_costcode = v_loopcostcode loop
                   ...
                   If c_loa%ISOPEN Then
                      ...
                      loop
                         ...
                         if ((v_cumm_ticbhrs-v_cumm_ticb_alloc)+(v_cumm_subchrs-v_cumm_subc_alloc) > 0) and (v_budgetted-(v_ticbconsumed+v_subcconsumed+v_delta_hours+v_delta_current) > 0) then
	                         ...
	                         declare
  	                       	  v_old_rate number(1);
    	                     begin
      	                      select nvl(old_rate,0) into v_old_rate from inv_loa_master where projno = v_loopprojno and suborder_no = v_suborder_no and loa_no = v_loa_no;
              									 ...
              									 exception
                  									when no_data_found then
                       									...
                       									exception
                          									when no_data_found then
                               										v_rate_code1 := 'Z1';
                               										select old_rates into v_rates1 from inv_ratemaster where rate_code = v_rate_code1;
                               										v_rate_code2 := 'Z2';
                               										select old_rates into v_rates2 from inv_ratemaster where rate_code = v_rate_code2;
                               										v_rate_code3 := 'Z3';
	                               									select old_rates into v_rates3 from inv_ratemaster where rate_code = v_rate_code3;
  	                             									v_rate_code4 := 'Z4';
  		                           									select old_rates into v_rates4 from inv_ratemaster where rate_code = v_rate_code4;
      	           									   	end;
       													 end;
	        	                  end if;	
            	             end;   
                            ...
                            IF v_budgetted-(v_ticbconsumed+v_subcconsumed+v_delta_hours+v_delta_current) < (v_cumm_ticbhrs-v_cumm_ticb_alloc)+(v_cumm_subchrs-v_cumm_subc_alloc) Then
                               -- if balance budget is less than balance consummed to allocate (Only TICB)
                               IF v_budgetted-(v_ticbconsumed+v_subcconsumed+v_delta_hours+v_delta_current) < (v_cumm_ticbhrs-v_cumm_ticb_alloc) Then   
                                  ...
                                  v_revised,v_ticbconsumed,v_subcconsumed,v_invoiced,v_budgetted-(v_ticbconsumed+v_subcconsumed+v_delta_hours+v_delta_current),0.00,
                                  v_budgetted-(v_ticbconsumed+v_subcconsumed+v_delta_hours+v_delta_current),v_rate_code,v_rate,0,p_processing_month,v_invoice_no,v_tcm_no,
                               ...
                               Else
                                  ...
                                  rollback_current,rollback_current_amount,additional) 
                                  values(v_loopprojno,v_suborder_no,v_loa_no,v_loopcostcode,v_budgetted,
                                  v_revised,v_ticbconsumed,v_subcconsumed,v_invoiced,v_cumm_ticbhrs-v_cumm_ticb_alloc,(v_budgetted-(v_ticbconsumed+v_subcconsumed+v_delta_hours+v_delta_current))-(v_cumm_ticbhrs-v_cumm_ticb_alloc),
                                  ...
                                  v_cumm_ticb_alloc := v_cumm_ticb_alloc + (v_cumm_ticbhrs-v_cumm_ticb_alloc);
                                  v_cumm_subc_alloc := v_cumm_subc_alloc + ((v_budgetted-(v_ticbconsumed+v_subcconsumed+v_delta_hours+v_delta_current))-(v_cumm_ticbhrs-v_cumm_ticb_alloc));
                               End IF;
                            Else   
                               ...
                               rollback_current,rollback_current_amount,additional) 
                               values(v_loopprojno,v_suborder_no,v_loa_no,v_loopcostcode,v_budgetted,
                               v_revised,v_ticbconsumed,v_subcconsumed,v_invoiced,v_cumm_ticbhrs-v_cumm_ticb_alloc,v_cumm_subchrs-v_cumm_subc_alloc,
                               v_cumm_ticbhrs-v_cumm_ticb_alloc,v_rate_code,v_rate,0,p_processing_month,v_invoice_no,v_tcm_no,
                               v_short_code,0,null,v_delta_hours,v_delta_current,
                               v_invoiced_amount,v_delta_current_amount,v_rollback_overbudget,v_rollback_overbudget_amount,
                               v_rollback_current,v_rollback_current_amount,0);
                               ...
                               v_cumm_ticb_alloc := v_cumm_ticb_alloc + (v_cumm_ticbhrs-v_cumm_ticb_alloc);
                               v_cumm_subc_alloc := v_cumm_subc_alloc + (v_cumm_subchrs-v_cumm_subc_alloc);
                            End IF;   
                        end if;       
                     --Else
                   ...
                   If (v_cumm_ticbhrs-v_cumm_ticb_alloc) > 0.00  and v_loa_found Then
                      ...
                      --- allocate over budget manhours againt dummy LOA
		                  update inv_workfile1 set ticb_current = ticb_current + (v_cumm_ticbhrs-v_cumm_ticb_alloc),
                             subc_current = subc_current + (v_cumm_subchrs-v_cumm_subc_alloc),
                             invoiced_current = invoiced_current + (v_cumm_ticbhrs-v_cumm_ticb_alloc),
                   ...
                   End If;
		               /* not in use 
		               v_total_ticb := v_total_ticb + v_cumm_ticb_alloc;
                   v_total_subc := v_total_subc + v_cumm_subc_alloc;
                   v_total_inv  := v_total_inv + v_cumm_ticb_alloc;
                   */
                   Fetch c_cumm into v_projno,v_costcode,v_cumm_ticbhrs,v_cumm_subchrs;
