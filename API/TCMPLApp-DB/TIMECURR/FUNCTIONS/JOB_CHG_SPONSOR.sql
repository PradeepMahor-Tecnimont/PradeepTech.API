--------------------------------------------------------
--  DDL for Function JOB_CHG_SPONSOR
--------------------------------------------------------

  CREATE OR REPLACE FUNCTION "TIMECURR"."JOB_CHG_SPONSOR" (
...
-- Returns -2 when Job not available in system
    select projno,actual_closing_date,pm_empno,incharge into v_projno ,v_cdate,v_pm_empno, v_sponsor from jobmaster where projno = param_projno;
      if v_sponsor <>  param_empno then
          If v_projno = param_projno Then
             if v_cdate is  null  then
                select empno,dol ,abbr into v_empno,v_dol,v_abbr from emplmast where empno = param_empno and job_incharge = 1;
                if v_empno = param_empno then
                   if v_dol is null then 
                      update jobmaster set incharge = param_empno  where  projno = param_projno;
                      update projmast set prjdymngr = param_empno  where  proj_no =  param_projno;
