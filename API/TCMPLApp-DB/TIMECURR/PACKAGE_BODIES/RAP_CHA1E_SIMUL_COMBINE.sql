Create Or Replace Package Body "TIMECURR"."RAP_CHA1E_SIMUL_COMBINE" As

    Procedure rpt_cha1_expt_simul(p_yyyy       In  Varchar2,
                                  p_yymm       In  Varchar2,
    ...
    Begin
        ...
        -- General Data --------------------------------------------------------        
        Open p_gen For
            'Select a.costcode, a.abbr, 
			sum(nvl(a.noofemps,0)) AS noofemps,
                     ...
                     sum(nvl(a.noofcons,0)) as noofcons 
		    from costmast a, 
                         emplmast b
                   where a.hod = b.empno and costcode = :p_costcode 
	    Group by a.costcode, a.abbr, b.name'
            Using p_costcode;
            ...
            'select * from (
                    ...
                    select * from (
                      with t_yymm as ( select yymm from table(rap_reports.rpt_month_cols(:v_new_yymm, :noofmonths)) ),
                         ...
                         from rap_gtt_movemast_combined where costcode = :p_costcode and yymm > :p_yymm order by yymm )
        ...
        Open p_future For
            'select * from
                    ...
                    select (select x.newcostcode from exptjobs x where x.projno = b.projno and (x.active = 1 or x.activefuture = 1) ' ||
            mfilter || ') newcostcode,
                    b.projno, a.yymm, b.hrs from t_yymm a, t_data b where a.yymm = b.yymm(+) order by yymm
...
End rap_cha1e_simul_combine;
/
Grant Execute On "TIMECURR"."RAP_CHA1E_SIMUL_COMBINE" To "TCMPL_APP_CONFIG";