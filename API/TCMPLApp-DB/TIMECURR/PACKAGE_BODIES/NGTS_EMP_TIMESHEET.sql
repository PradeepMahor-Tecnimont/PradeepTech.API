--------------------------------------------------------
--  DDL for Package Body NGTS_EMP_TIMESHEET
--------------------------------------------------------

  CREATE OR REPLACE PACKAGE BODY "TIMECURR"."NGTS_EMP_TIMESHEET" AS
  ...
  FUNCTION ts_delete_status (
   ...
   vntcount   NUMBER;
        votcount   NUMBER;
    BEGIN
        SELECT COUNT(*)  INTO vntcount
        FROM   time_daily
			WHERE  substr(yymm, 0, 4) = p_year  AND
				   empno = p_empno;

        SELECT  COUNT(*) INTO votcount
		FROM  time_ot
			WHERE   substr(yymm, 0, 4) = p_year AND 
				empno = p_empno;
...
PROCEDURE ngts_GetReasonCode (
    ...
    select count(*) into v_count from ngts_proj_reasoncode WHERE    projno = substr(P_projno , 1, 5);
    ...
    if v_count > 0 then    
            ...
            v_stmt := '   select ''NA'' AS code, ''Not Applicable'' AS "desc" from dual ';
    end if;
  ...
  PROCEDURE ngts_gettimesheet (        
    ...
    BEGIN
        ...
        v_mst_stmt := v_mst_stmt || ' TM.grp, TM.tot_nhr, TM.tot_ohr, TM.remark , TM.MASTKEYID ID';        
		...
		where    projno = TD.projno and   WPCODE = TD.WPCODE and  ACTIVITY = TD.ACTIVITY and 
				yymm='''|| p_yymm || ''' and 
				assign=''' || p_costcode || '''  and 
				empno=''' || p_empno || '''  
				) as Daily_hours , TD.DAILYKEYID ID , TD.MASTKEYID , TD.REASONCODE ';
				-- DAILYKEYID , MASTKEYID
		...
		where   projno = OT.projno and   WPCODE = OT.WPCODE and  ACTIVITY = OT.ACTIVITY   and 
				...
				assign='''|| p_costcode || '''  and 
				empno='''|| p_empno || '''   
				) as OT_hours , OT.OTKEYID id , OT.MASTKEYID ,OT.REASONCODE ';     
				-- OTKEYID , MASTKEYID
  ...
  v_stmt    VARCHAR2(1000) := NULL;
    ...
    BEGIN
        ...
        FROM   NGTS_VUSER_DEPT_ROLES
        WHERE  deptno = p_dept AND 
			   empno = p_empno;
  ...
  PROCEDURE ngts_getAllemplist (
        p_empno                IN    VARCHAR2,
--        p_dept                 IN    VARCHAR2,
        p_allemplist   OUT   SYS_REFCURSOR
    ) AS
  v_stmt    VARCHAR2(1000) := NULL;
        v_count   NUMBER;
    BEGIN
        ...
        -------For Proxy - HOD-------
            ...
            v_stmt := v_stmt || '  (select DISTINCT abbr  from costmast where costcode  = E.parent ) parentabbr ';
            v_stmt := v_stmt || ' FROM    emplmast E ';
            v_stmt := v_stmt || ' where E.status=1 order by name ';
            --v_stmt := v_stmt || ' and E.parent=''' || p_dept || ''' ';
        ELSE
        -------For HOD-------
           v_stmt := ' SELECT  DISTINCT  E.empno, E.name ,  E.email,    E.parent  ,E.COMPANY ,E.DESGCODE , E.GRADE , ';
        ...
        OPEN p_allemplist FOR v_stmt;
  END ngts_getAllemplist;
    ...
    BEGIN
        ...
        RETURN o_cursor;
    END ngts_getprojtimesheetdetails;
...
BEGIN
	IF p_projno IS NOT NULL THEN                                                    
		p_whlist := ngts_getWorkingHours(p_yymm);
	...
	v_stmt := v_stmt || ' ngts_Emp_TimeSheet.Get_emp_Name(TT.empno) Employee, ';
	...
	v_stmt := v_stmt || ' (	select NVL( sum(NT.TOTAL), ''0'') 
									from time_daily NT 
								where NT.yymm=TT.yymm and
								NT.empno= TT.empno and 
								NT.assign =trim(''' || p_dept || ''') 
							) NT_TOTAL ,   ';
	v_stmt := v_stmt || ' ( select NVL( sum(OT.TOTAL), ''0'') 
									from time_OT OT 
								where OT.yymm=TT.yymm and
								OT.empno= TT.empno and
								OT.assign = trim(''' || p_dept || ''') 
						    ) OT_TOTAL ,   ';
	v_stmt := v_stmt || ' ( select NVL( sum(NT.TOTAL), ''0'') 
									from time_daily NT 
								where NT.yymm=TT.yymm and 
	...
	v_stmt := v_stmt || ' (select DISTINCT abbr  from costmast where costcode  = E.parent ) parentabbr  , TT.MASTKEYID ID';
...
/* */
	 ...
	 v_stmt := v_stmt || ' ngts_Emp_TimeSheet.Get_emp_Name(a.empno) Employee ,   ';
	 ...
	 v_stmt := v_stmt || ' ( select DISTINCT abbr  from costmast where costcode  = a.parent ) parentabbr   ,null ID ';
  ...
  v_stmt    VARCHAR2(1000) := NULL;
    ...
    BEGIN
        ...
        v_stmt := v_stmt || '       ngts_Emp_TimeSheet.Get_total(''NT'',TT.yymm,TT.empno, TT.parent,TT.assign,''Proj'')NT_TOTAL , ';
        v_stmt := v_stmt || '       ngts_Emp_TimeSheet.Get_total(''OT'',TT.yymm,TT.empno, TT.parent,TT.assign,''Proj'')OT_TOTAL , '; 
        v_stmt := v_stmt || '        TT.company,     case when TT.parent =  TT.assign then ''Normal'' else ''ODD'' end Type ,';
        v_stmt := v_stmt || '       TT.grp,  TT.tot_nhr,  TT.tot_ohr   ,  TT.MASTKEYID ID ';
        v_stmt := v_stmt || '    FROM  ';
 ...
 npmapproved NUMBER(1);
    BEGIN
        SELECT   pmapproved  INTO npmapproved
        FROM
           (  SELECT  projno, pmapproved
			  FROM (  SELECT  
							projno,
            ...
            WHERE
			EXISTS ( SELECT projmast.*  FROM  projmast p
                        WHERE  
 ...
 PROCEDURE ngts_gettimesheet_by_id (
        p_KeyId         IN    VARCHAR2,
        p_time_master   OUT   SYS_REFCURSOR,
        p_time_daily    OUT   SYS_REFCURSOR,
        p_time_ot       OUT   SYS_REFCURSOR,
    ...
    BEGIN
        ...
        v_mst_stmt := v_mst_stmt || ' TM.grp, TM.tot_nhr, TM.tot_ohr, TM.remark , TM.MASTKEYID ID';        
        v_mst_stmt := v_mst_stmt || ' FROM time_mast TM ';                
        v_mst_stmt := v_mst_stmt || ' where TM.MASTKEYID='''|| p_KeyId ||''' ';             
-------------------------------------------------------------------------------------------------------------
		...
		SELECT  ''[''|| 
			...
			nvl(d31,''0'') || '']''
        FROM    time_daily   
		where   MASTKEYID = TD.MASTKEYID and  projno = TD.projno
                 and   WPCODE = TD.WPCODE and  ACTIVITY = TD.ACTIVITY and 
				yymm=TD.yymm and 
				assign=TD.assign  				
				) as Daily_hours , TD.DAILYKEYID ID , TD.MASTKEYID , TD.REASONCODE ';
            v_daily_stmt := v_daily_stmt || ' FROM time_daily TD, projmast P ';                    
            v_daily_stmt := v_daily_stmt || ' where td.projno = p.projno and TD.MASTKEYID='''|| p_KeyId ||''' ';             
...
-------------------------------------------------------------------------------------------------------------------------------------------          
		...
		SELECT  ''[''|| 
			...
			nvl(d31,''0'') || '']''
        FROM    time_ot  
		where   MASTKEYID = OT.MASTKEYID and  projno = OT.projno
                 and   WPCODE = OT.WPCODE and  ACTIVITY = OT.ACTIVITY and 
				yymm=OT.yymm and 
				assign=OT.assign  	
				) as OT_hours , OT.OTKEYID id , OT.MASTKEYID ,OT.REASONCODE ';     
        v_ot_stmt := v_ot_stmt || ' FROM    time_ot  OT, projmast P ';                  
        v_ot_stmt := v_ot_stmt || ' where OT.projno = p.projno and  OT.MASTKEYID='''|| p_KeyId ||''' ';             
        ...
        v_total_stmt := v_total_stmt || ' FROM time_mast TM ';
        v_total_stmt := v_total_stmt || ' where TM.MASTKEYID='''|| p_KeyId  || ''' '; 
         ...
         OPEN p_Total FOR v_total_stmt;
  END ngts_gettimesheet_by_id;

END ngts_Emp_TimeSheet;
