--------------------------------------------------------
--  DDL for View MARCHHRS
--------------------------------------------------------

  CREATE OR REPLACE FORCE VIEW "TIMECURR"."MARCHHRS" ("COSTCODE", "PROJ_TYPE", "TOTHOURS") AS 
  ((SELECT A.COSTCODE,'TCM' AS PROJ_TYPE,SUM(NVL(A.HOURS,0))+SUM(NVL(A.OTHOURS,0)) AS TOTHOURS FROM TIMETRAN A,PROJMAST B
WHERE A.PROJNO = B.PROJNO AND A.YYMM = '200203' AND B.TCM_JOBS = 1 GROUP BY A.COSTCODE)
UNION
(SELECT A.COSTCODE,'NON TCM'AS PROJ_TYPE,SUM(NVL(A.HOURS,0))+SUM(NVL(A.OTHOURS,0)) AS TOTHOURS FROM TIMETRAN A,PROJMAST B
WHERE A.PROJNO = B.PROJNO AND A.YYMM = '200203' AND A.PROJNO NOT IN('1111400','2222400','5555400','6666400')
...
A.YYMM = '200203' AND A.COSTCODE IN('0236','0237') GROUP BY B.PARENT)
union
