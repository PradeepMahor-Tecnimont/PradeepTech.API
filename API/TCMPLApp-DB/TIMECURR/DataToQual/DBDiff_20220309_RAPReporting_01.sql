...
--RAP_REPORTS_GEN
---------------------------
CREATE OR REPLACE PACKAGE "TIMECURR"."RAP_REPORTS_GEN" as 
    c_mail_from    CONSTANT VARCHAR2(35) := 'TIMESHEET';
    c_path         CONSTANT VARCHAR2(60) := 'Dashboard > Analytical Reports > Costcenter Reports';

  function get_empcount(p_costcode in varchar2) return number;
  function get_lastmonth(p_yymm in varchar2, p_mnths in number) return varchar2;

      --Get Begin Year Month        
...
--TASK_SHCEDULER
---------------------------
CREATE OR REPLACE PACKAGE BODY "TIMECURR"."TASK_SHCEDULER" As
...
CREATE OR REPLACE PACKAGE BODY "TIMECURR"."RAP_REPORTS_C" as
  ...
  procedure rpt_cha1e_cc(p_yymm in varchar2, p_costcode in varchar2, p_simul in varchar2, p_cols out sys_refcursor, 
                         p_gen out sys_refcursor, p_alldata out sys_refcursor, p_ot out sys_refcursor, 
                         p_project out sys_refcursor, p_future out sys_refcursor) as
    pivot_clause varchar2(4000);
    mfilter varchar2(1000);
  ...
  begin
    ...
    open p_alldata for
      'select * from (
        with t_yymm as ( select yymm from table(rap_reports.rpt_month_cols(:p_yymm, :noofmonths)) ),
           t_data as ( select yymm, ''a'' as Name, nvl(sum(working_hr),0) hrs from raphours where yymm >= :p_yymm 
                       group by yymm, ''a'' union
                       select yymm, ''b'' as Name, nvl(sum(fut_recruit),0) hrs from rap_tab_movemast where yymm >= :p_yymm 
                       and costcode = :p_costcode group by yymm, ''b'' union                   
                       select yymm, ''c'' as Name, nvl(sum(int_dept),0) hrs from rap_tab_movemast where yymm >= :p_yymm 
                       and costcode = :p_costcode group by yymm, ''c'' union 
                       select yymm, ''d'' as Name, nvl(sum(movetotcm),0) hrs from rap_tab_movemast where yymm >= :p_yymm 
                       and costcode = :p_costcode group by yymm, ''d'' union
                       select yymm, ''e'' as Name, nvl(sum(movetosite),0) hrs from rap_tab_movemast where yymm >= :p_yymm 
                       and costcode = :p_costcode group by yymm, ''e'' union
                       select yymm, ''f'' as Name, nvl(sum(movetoothers),0) hrs from rap_tab_movemast where yymm >= :p_yymm 
                       and costcode = :p_costcode group by yymm, ''f'' union
                       select yymm, ''g'' as Name, nvl(sum(ext_subcontract),0) hrs from rap_tab_movemast where yymm >= :p_yymm 
                       and costcode = :p_costcode group by yymm, ''g'')
        select a.yymm, b.name, b.hrs from t_yymm a, t_data b where a.yymm = b.yymm(+) and name is not null order by yymm
       ) pivot (sum(hrs) for yymm in ('|| pivot_clause ||')) order by name'
    using p_yymm, noofmonths, p_yymm, p_yymm, p_costcode, p_yymm, p_costcode, p_yymm, 
    p_costcode, p_yymm, p_costcode, p_yymm, p_costcode, p_yymm, p_costcode;
    -- Overtime ------------------------------------------------------------
    open p_ot for
      'select * from (
        with t_yymm as ( select yymm from table(rap_reports.rpt_month_cols(:p_yymm, :noofmonths))  ),
        t_data as ( select yymm, ot from otmast where costcode = :p_costcode and yymm >= :p_yymm )
        select a.yymm, b.ot from t_yymm a, t_data b where a.yymm = b.yymm(+) order by yymm
      ) pivot (sum(ot) for yymm in ('|| pivot_clause ||'))'
    using p_yymm, noofmonths, p_costcode, p_yymm;  
    -- Projects ------------------------------------------------------------
    open p_project for
      'select row_number() over (order by q.newcostcode, p.projno) srno, q.*, p.* from (
        with t_yymm as ( select yymm from table(rap_reports.rpt_month_cols(:p_yymm, :p_costcode)) ),
        t_data as ( select projno, yymm, nvl(hours,0) as hrs from prjcmast where costcode = :p_costcode 
                      and yymm >= :p_yymm )
        select b.projno, a.yymm, b.hrs from t_yymm a, t_data b where a.yymm = b.yymm(+) order by yymm
      ) pivot (sum(hrs) for yymm in ('|| pivot_clause ||'))
      p join (select projno, name, active, cdate, newcostcode, tcmno from projmast 
    ...
    open p_future for
      'select row_number() over (order by q.active,q.newcostcode, p.projno) srno, q.*, p.* from (
        with t_yymm as ( select yymm from table(rap_reports.rpt_month_cols(:p_yymm, :p_costcode)) ),
        t_data as ( select projno, yymm, nvl(hours,0) as hrs from exptprjc where costcode = :p_costcode 
                      and yymm >= :p_yymm order by yymm )
        select b.projno, a.yymm, b.hrs from t_yymm a, t_data b where a.yymm = b.yymm(+) order by yymm
      ) pivot (sum(hrs) for yymm in ('|| pivot_clause ||'))
      p join (select projno, name, active, activefuture, newcostcode, tcmno from exptjobs 
  ...
  procedure rpt_cha1e_category(p_yymm in varchar2, p_category in varchar2, p_simul in varchar2,p_mnths in varchar2,  
                               p_cols out sys_refcursor, p_gen out sys_refcursor, p_alldata out sys_refcursor,  
                               p_ot out sys_refcursor, p_project out sys_refcursor, p_future out sys_refcursor) as
    pivot_clause varchar2(4000);
  ...
  begin
    ...
    select listagg( yymm || ' as "' || heading || '"', ', ') within group (order by yymm) into pivot_clause
    from (select yymm, heading from table(rap_reports.rpt_month_cols(p_yymm, p_mnths)));    
    -- General Data --------------------------------------------------------
    open p_gen for     
      'select sum(noofemps) noofemps, sum(changed_nemps) changed_nemps, sum(noofcons) noofcons,
        to_char(sysdate, ''dd-Mon-yyyy'') pdate from costmast 
        where ' || str_costcode;    
    -- Column names --------------------------------------------------------
    open p_cols for 
      'select * from ( 
        select yymm from table(rap_reports.rpt_month_cols(:p_yymm, :p_mnths))
      ) pivot (max(yymm) for yymm in ('|| pivot_clause ||'))'
    using p_yymm, p_mnths; 
    -- All Data  --------------------------------------------------
    open p_alldata for 
      'select * from (
        with t_yymm as ( select yymm from table(rap_reports.rpt_month_cols(:p_yymm, :p_mnths)) ),
           t_data as ( select yymm, ''a'' as Name, nvl(sum(working_hr),0) hrs from raphours  
                        where yymm >= :p_yymm group by yymm, ''a''                                  union
                       select yymm, ''b'' as Name, nvl(sum(fut_recruit),0) hrs from rap_tab_movemast 
                         where yymm >= :p_yymm and ' || str_costcode || ' group by yymm, ''b''      union                   
                       select yymm, ''c'' as Name, nvl(sum(int_dept),0) hrs from rap_tab_movemast  
                         where yymm >= :p_yymm and ' || str_costcode || ' group by yymm, ''c''      union 
                       select yymm, ''d'' as Name, nvl(sum(movetotcm),0) hrs from rap_tab_movemast  
                         where yymm >= :p_yymm and ' || str_costcode || ' group by yymm, ''d''      union
                       select yymm, ''e'' as Name, nvl(sum(movetosite),0) hrs from rap_tab_movemast  
                         where yymm >= :p_yymm and ' || str_costcode || ' group by yymm, ''e''      union
                       select yymm, ''f'' as Name, nvl(sum(movetoothers),0) hrs from rap_tab_movemast 
                         where yymm >= :p_yymm and ' || str_costcode || ' group by yymm, ''f''      union
                       select yymm, ''g'' as Name, nvl(sum(ext_subcontract),0) hrs from rap_tab_movemast  
                         where yymm >= :p_yymm and ' || str_costcode || ' group by yymm, ''g'')
        select a.yymm, b.name, b.hrs from t_yymm a, t_data b where a.yymm = b.yymm(+) and name is not null order by yymm
      ) pivot (sum(hrs) for yymm in ('|| pivot_clause ||')) order by name'
    using p_yymm, p_mnths, p_yymm, p_yymm, p_yymm, p_yymm, p_yymm,  p_yymm, p_yymm;
    -- Overtime ------------------------------------------------------------
    open p_ot for 
      'select * from (
        with t_yymm as ( select yymm from table(rap_reports.rpt_month_cols(:p_yymm, :p_mnths))  ),
    ...
    using p_yymm, p_mnths, p_yymm;
    -- Projects ------------------------------------------------------------
    open p_project for 
      'select row_number() over (order by q.newcostcode, p.projno) srno, q.*, p.* from (
        with t_yymm as ( select yymm from table(rap_reports.rpt_month_cols(:p_yymm, :p_mnths)) ),
        t_data as ( select projno, yymm, nvl(hours,0) as hrs from prjcmast where '|| str_costcode || ' 
                      and yymm >= :p_yymm )
        select b.projno, a.yymm, b.hrs from t_yymm a, t_data b where a.yymm = b.yymm(+) order by yymm
      ) pivot (sum(hrs) for yymm in ('|| pivot_clause ||'))
      p join (select projno, name, active, cdate, newcostcode, tcmno from projmast 
      where active > 0 and projno in (select distinct projno from prjcmast 
        where yymm > :p_yymm and '|| str_costcode || ')) q on p.projno = q.projno
        order by q.newcostcode, p.projno ' 
    using p_yymm, p_mnths, p_yymm, p_yymm;
    -- Future ------------------------------------------------------------
    open p_future for 
      'select row_number() over (order by q.active,q.newcostcode, p.projno) srno, q.*, p.* from (
        with t_yymm as ( select yymm from table(rap_reports.rpt_month_cols(:p_yymm, :p_mnths)) ),
        t_data as ( select projno, yymm, nvl(hours,0) as hrs from exptprjc where '|| str_costcode || ' 
                      and yymm >= :p_yymm  )
        select b.projno, a.yymm, b.hrs from t_yymm a, t_data b where a.yymm = b.yymm(+) order by yymm
      ) pivot (sum(hrs) for yymm in ('|| pivot_clause ||'))
      p join (select projno, name, active, activefuture, newcostcode, tcmno from exptjobs 
        ...
        when 'E' then 
          select listagg( tmagrp || ' as "' || tmagrp || '"', ', ') within group (order by tmagrp) into pivot_clause_m
            from (select distinct tmagrp from rap_vu_rpt_breakup where yymm = p_yymm and costcode in (select costcode 
              from costmast where tma_grp = 'E' and activity = 1 and group_chart = 1 and costcode like '02%' ));
          select listagg( tmagrp || ' as "' || tmagrp || '"', ', ') within group (order by tmagrp) into pivot_clause_p
            from (select distinct tmagrp from rap_vu_rpt_breakup where yymm >= startYYMM and yymm <= p_yymm and costcode in 
              (select costcode from costmast where tma_grp = 'E' and activity = 1 and group_chart = 1 and costcode like '02%' ));
          str_costcode := ' and costcode in (select costcode from costmast where tma_grp = ''E''
                            and activity = 1 and group_chart = 1 and costcode like ''02%'' )';          
        when 'N' then
          select listagg( tmagrp || ' as "' || tmagrp || '"', ', ') within group (order by tmagrp) into pivot_clause_m
            from (select distinct tmagrp from rap_vu_rpt_breakup where yymm = p_yymm and costcode in (select costcode 
              from costmast where tma_grp in ('P','M','C') and activity = 1 and group_chart = 1 and costcode like '02%'));
          select listagg( tmagrp || ' as "' || tmagrp || '"', ', ') within group (order by tmagrp) into pivot_clause_p
            from (select distinct tmagrp from rap_vu_rpt_breakup where yymm >= startYYMM and yymm <= p_yymm 
              ...
              and group_chart = 1 and costcode like '02%'));        
          ...
          select listagg( tmagrp || ' as "' || tmagrp || '"', ', ') within group (order by tmagrp) into pivot_clause_p
            from (select distinct tmagrp from rap_vu_rpt_breakup where yymm >= startYYMM and yymm <= p_yymm 
              and costcode in (select costcode from costmast where (tma_grp = 'E' or tma_grp = 'P' or tma_grp = 'C' 
                or tma_grp = 'M') and activity = 1 and group_chart = 1 and costcode like '02%'));        
          str_costcode := ' and costcode in (select costcode from costmast where (tma_grp = ''E'' 
                            or tma_grp = ''P'' or tma_grp = ''C'' or tma_grp = ''M'')
                            and activity = 1 and group_chart = 1 and costcode like ''02%'' )';
     end case;
