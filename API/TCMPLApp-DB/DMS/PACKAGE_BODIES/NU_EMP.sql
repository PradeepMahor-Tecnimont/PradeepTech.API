...
--------------------------------------------------------
  ...
  CREATE OR REPLACE PACKAGE BODY "NU_EMP" AS
  ...
  begin
      for nu_emp_row in cur_nu_emp loop
         ...
         If sysdate >= SelfService.NU_EMP.Get_assign_shift_run_date(to_char(add_months(sysdate,1), 'yyyymm')) Then
  ...
  Begin
      If Not (sysdate >= SelfService.NU_EMP.Get_assign_shift_run_date(param_yyyymm)) Then
          Return;
      End If;
