Create Or Replace Package Body dms.pkg_dm_area_type_user_mapping_qry As
                     ...
                     From (
                       Select a.area_key_id                                  As area_id,
                        ...
                        Where c.area_catg_code = a.area_catg_code
                          ...
                          And (upper(c.description) Like '%' || upper(Trim(p_generic_search)) || '%'
                   ...
                   total_row
                     From (
                       Select empno                         empno,
                              ...
                              Count(*) Over()               total_row
                         From (
                                  Select ss_emplmast.empno                     As empno,
                                   ...
                                   Where ss_emplmast.status = 1
                              )
                        Where nvl(area_id, 'X!X!') = nvl(Trim(p_area_id), nvl(area_id, 'X!X!'))
                          And nvl(office_location_code, 'X!X!') = nvl(Trim(p_office_location_code), nvl(office_location_code,
                          'X!X!'))
                          And dept_code = nvl(Trim(p_cost_code), dept_code)
                          And (upper(dept_code) Like '%' || upper(Trim(p_generic_search)) || '%'
                           Or upper(empno) Like '%' || upper(Trim(p_generic_search)) || '%'
    ...
    Begin
        ...
        Select Count(*)
          Into v_exists
          From dm_area_type_user_mapping
        ...
        If v_exists = 1 Then
            Select b.area_catg_code                                      As area_catg_code,
              ...
              Into p_area_catg_code,
                   ...
                   p_modified_on,
                   p_modified_by
              From dm_area_type_user_mapping a,
                   dm_desk_areas b,
                   ...
                   From (
                     With tags As (
                       ...
                       Select empno                         empno,
                              ...
                              booked_desk                   As booked_desk,
                              desk_book_in_office           As desk_book_in_office,
                              Row_Number() Over(Order By 1) row_number,
                              Count(*) Over()               total_row
                         From (
                                  Select ss_emplmast.empno                     As empno,
                                   ...
                                   Inner Join ss_costmast
                                    ...
                                    Left Outer Join desk_book.db_desk_bookings desk_book
                                      On desk_book.empno = ss_emplmast.empno and desk_book.attendance_date = trunc(sysdate)
                                   Where ss_emplmast.status = 1
                              ),tags tg
                        Where area_id = tg.area_key_id (+)
                        And nvl(area_id, 'X!X!') = nvl(Trim(p_area_id), nvl(area_id, 'X!X!'))
                          And nvl(office_location_code, 'X!X!') = nvl(Trim(p_office_location_code), nvl(office_location_code,
                          'X!X!'))
                          And dept_code = nvl(Trim(p_cost_code), dept_code)
                          And (upper(dept_code) Like '%' || upper(Trim(p_generic_search)) || '%'
...
End pkg_dm_area_type_user_mapping_qry;