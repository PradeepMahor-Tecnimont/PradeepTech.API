...
namespace TCMPLApp.Library.Excel.Template
{
    public partial class ExcelTemplate : IExcelTemplate
    {
        ...
        #region Import
        public MemoryStream ValidateDMSInvGroupItems(Stream stream, IEnumerable<ValidationItem> validationItems)
        {
        ...
        public MemoryStream ExportDMSInvGroupItems(string version, DictionaryCollection dictionaryCollection, int rowLimit)
        {
            ...
            var filename = string.Format("{0} {1}.xlsx", "ImportDMSGroupItems", version);
            ...
            using (var resourceStream = assembly.GetManifestResourceStream(resourceName))
            {
                ...
                using (var ms = new MemoryStream())
                {
                    ...
                    var columnReason = worksheetImport.Names["ItemId"];
        ...
        {
            List<InvGroupItem> invGroupItems = new List<InvGroupItem>();
            stream.Position = 0;
                ...
                while (string.IsNullOrEmpty(worksheet.Cells[rowIndex, 1].GetValue<string>()) == false)
                {
                    ...
                    foreach (var prop in props)
                    {
                        ...
                        if (worksheet.Cells[rowIndex, column.Start.Column].Value != null)
                        {
                            ...
                            {
                                pinfo?.SetValue(groupItem, worksheet.Cells[rowIndex, column.Start.Column].Value);
                            }
                            else
                            {
                                pinfo?.SetValue(groupItem, Convert.ChangeType(worksheet.Cells[rowIndex, column.Start.Column].Value, pinfo.PropertyType));
                            }
                        }

                    }
                    invGroupItems.Add(groupItem);
                    rowIndex += 1;
                }

            }

            return invGroupItems;
        }
