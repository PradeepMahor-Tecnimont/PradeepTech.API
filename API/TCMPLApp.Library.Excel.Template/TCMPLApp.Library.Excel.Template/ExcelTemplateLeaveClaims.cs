...
namespace TCMPLApp.Library.Excel.Template
{
    public partial class ExcelTemplate : IExcelTemplate
    {
        ...
        public MemoryStream ExportLeaveClaims(string version, DictionaryCollection dictionaryCollection, int rowLimit)
        {
            ...
            var filename = string.Format("{0} {1}.xlsx", "ImportLeaveClaims", version);
            ...
            using (var resourceStream = assembly.GetManifestResourceStream(resourceName))
            {
                ...
                using (var ms = new MemoryStream())
                {
                    ...
                    var columnReason = worksheetImport.Names["Reason"];
                    ...
                    validationReason.Error = "Reason length should be less than 30 characters";
                    validationReason.ShowErrorMessage = true;
                    validationReason.Formula.Value = 30;
        ...
        {
            List<LeaveClaim> LeaveClaims = new List<LeaveClaim>();
            stream.Position = 0;
                ...
                while (string.IsNullOrEmpty(worksheet.Cells[rowIndex, 1].GetValue<string>()) == false)
                {
                    ...
                    foreach (var prop in props)
                    {
                        ...
                        if (worksheet.Cells[rowIndex, column.Start.Column].Value != null)
                        {
                            ...
                            {
                                pinfo?.SetValue(LeaveClaim, worksheet.Cells[rowIndex, column.Start.Column].Value);
                            }
                            else
                            {
                                pinfo?.SetValue(LeaveClaim, Convert.ChangeType(worksheet.Cells[rowIndex, column.Start.Column].Value, pinfo.PropertyType));
                            }
                        }

                    }
                    LeaveClaims.Add(LeaveClaim);
                    rowIndex += 1;
                }
