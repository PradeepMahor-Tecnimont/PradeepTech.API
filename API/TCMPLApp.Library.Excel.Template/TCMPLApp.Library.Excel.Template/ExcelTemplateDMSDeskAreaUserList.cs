...
namespace TCMPLApp.Library.Excel.Template
{
    public partial class ExcelTemplate : IExcelTemplate
    {
        ...
        public MemoryStream ExportDMSDeskAreaUserList(string version, DictionaryCollection dictionaryCollection, int rowLimit)
        {
            ...
            var filename = string.Format("{0} {1}.xlsx", "ImportDeskAreaUserList", version);
        ...
        {
            List<DeskAreaUserList> users = new List<DeskAreaUserList>();
            stream.Position = 0;
                ...
                while (string.IsNullOrEmpty(worksheet.Cells[rowIndex, 1].GetValue<string>()) == false)
                {
                    ...
                    foreach (var prop in props)
                    {
                        ...
                        if (worksheet.Cells[rowIndex, column.Start.Column].Value != null)
                        {
                            ...
                            {
                                pinfo?.SetValue(userRow, worksheet.Cells[rowIndex, column.Start.Column].Value);
                            }
                            else
                            {
                                pinfo?.SetValue(userRow, Convert.ChangeType(worksheet.Cells[rowIndex, column.Start.Column].Value, pinfo.PropertyType));
                            }
                        }
